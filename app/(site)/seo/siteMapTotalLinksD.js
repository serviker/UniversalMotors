...
const SiteMapTotalLinksD = (props) => {
    ...
    useEffect(() => {
        const fetchData = async (page) => {
            try {
                const response = await axios.get(`http://185.25.10.105:5002/seositemap?page=${page}&size=${pageSize}&d=d${category ? `&category=${category}` : ''}`);
    ...
    return (
        <>
            <h4>Задублированные ссылки в карте сайта</h4>
            {totalPages > 1 && (
            ...
            <Table striped bordered hover>
                ...
                {totalLink.map((item, index) => (
                    <tr key={index}>
                        <td> <a href={item.NameLink.replace('https://universalmotors.ru/', 'https://office.universalmotors.ru/')} target="_blank" rel="noopener noreferrer">
                            {item.NameLink}
                        </a></td>
...
export default SiteMapTotalLinksD;
