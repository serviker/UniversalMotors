...
const Imgdatasrc = (props) => {
    ...
    useEffect(() => {
        const fetchData = async (page) => {
            ...
            try {
                const response = await axios.get(`http://185.25.10.105:5002/seositemap?page=${page}&size=${pageSize}&err=imgdatasrc${category ? `&category=${category}` : ''}`);
                    ...
                    <Table striped bordered hover style={{ width: '100%', tableLayout: 'fixed' }}>
                        ...
                        {totalLink && totalLink.map((item, index) => (
                            <tr key={index}>
                                ...
                                <td style={{wordWrap: 'break-word'}}><a
                                    href={item.NameLink.replace('https://universalmotors.ru/', 'https://admin.universalmotors.ru/')}
