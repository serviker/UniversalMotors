...
const PageN = (props) => {
    ...
    useEffect(() => {
        const fetchData = async (page) => {
            ...
            try {
                const response = await axios.get(`http://185.25.10.105:5002/seositemap?page=${page}&size=${pageSize}&err=pageN${category ? `&category=${category}` : ''}`);
                    ...
                    <tr key={index}>
                        <td><a href={item.NameLink} target="_blank" rel="noopener noreferrer">
                            ...
                            <Button variant="primary" onClick={() => {
                                setLinks(item.NameLink);
                                handleModalOpen(item.NameLink);
